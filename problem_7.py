...
# For this exercise we are going to implement an HTTPRouter like you 
# would find in a typical web server using the Trie data structure we learned previously.
...
# "/", "/about", or "/blog/2019-01-15/my-awesome-blog-post" 
...
# used for autocomplete. Instead of simple words the Trie will contain a 
# part of the http path at each node, building from the root node /

# In addition to a path though, we need to know which function will handle 
# the http request. In a real router we would probably pass an instance of 
# a class like Python's SimpleHTTPRequestHandler which would be responsible 
# for handling requests to that path. For the sake of simplicity we will just 
...
# slashes ("/"). A Trie with a single path entry of: "/about/me" would look like:
...
# find nodes, and if a RouteTrieNode is not a leaf node, it won't have a handler
#  which is fine.
...
# Requests for '' or '/' are probably looking for the root handler. Handle these 
# edge cases in your Router.
